...
const EducationCard = ({ title, school, tags }: EducationCardProps) => (
  <div className="flex flex-col overflow-hidden p-3 rounded-lg bg-purple-100 relative w-full md:w-[250px] hover:bg-purple-200 transition-all duration-500">
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className="text-sm font-mavenPro">{school}</div>
...
const Circle = () => (
  <div className="hidden md:flex justify-center items-center sm:col-start-2 sm:col-end-3">
    <div className="text-3xl z-10 h-8 w-8 bg-[#F8D2FF] border-[#AA56B9] rounded-full items-center justify-center border-4"></div>
  </div>
);
      ...
      <div className="h-full flex items-center relative">
        <div className="h-full w-full absolute">
          ...
          {education.map((edu, index) => (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-center my-3" key={index}>
              {index % 2 === 0 ? (
                <>
                  <Circle />
